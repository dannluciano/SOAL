...
[6] the notes’ mode value (in mcs) per onset, i.e. the most frequent note on each onset. If not existing, the algorithm
...
[6] the velocities’ mode value per onset. If not existing, the algorithm returns back the average velocity (as in the 5th
ouput)."
...
(defmethod! pitch-direction ((notes list) (onsets list))
   :doc
"Gives the object’s directional profile, i.e. the global direction (descending and/or ascending) of the object’s sequence of events.
...
(defmethod! spatial-analysis ((notes list) (range-list number) (registers-list list) (register-weight-list list) (remove-duplicates? number))
   :doc
"Gives the input file’s RELATIVE RANGE and several register informations.
Range (soalambitus) means a pitch space defined by two boundary pitches.
Relative range compares the input object’s pitch range to a predefined global pitch range, which can be for instance the whole
...
the object’s pitch range. The user must declare in how many slices (registers) the global pitch range is to be partitionned ('registers' input)
— the default partition, by octaves, can be useful in many cases. Moreover, if he wants so, the user may define a quality weight
...
[2]  returns the file’s average spatial quality — i.e. the average value of [3] (relative pitch range) and [10] (register
distribution).
...
(defmethod! piano-spatial-analysis ((notes list) (remove-duplicates? number))
   :doc
"Gives the input file’s RELATIVE RANGE and several register informations. This patch is a version of Spatial Analysis optimized for piano music.
Relative range compares the input object’s pitch range to the piano global pitch range (88 semitons). Provides
complementary REGISTER informations, particularly the REGISTER DISTRIBUTION, which  informs how pitches are distributed along
the object’s pitch range. Seven piano registers have been defined according to some global timbral differences caused by
...
[3] returns “Q sum” (the most significant information) — the weighted sum of q values, including pedal multiplicator;
...
[7] returns the velocities mode — i.e. most frequent value (or, if not applicable, the average value) of the input file(s)’s
...
(defmethod! harmonicity ((notes list) (range number) (use-virtual-fundamental? number) (user-fundamental number)
                           (threshold number) (round-harmonic-object? number) (transpose-pitches? number) (remove-duplicates? number))
   :doc
"Evaluates the closeness of the input file’s achronic pitch distribution to a paradigmatic harmonic spectrum-like structure.
...
4 -> the mode ,i.e., the most frequent element of the list — if not
...
5 -> the mode ,i.e., the most frequent element of the list — if not existing, the algorithm goes back to the average value.
...
The default values — (2100) and (10800), respectively — are preset for piano music.
...
[4] the mode of list, i.e., the most frequent element of the list — if not existing, the algorithm goes back to the average value."
...
[4] the pitches' mode on each onset (midicent), i.e., the most frequent pitch on each onset — if not existing, the algorithm returns the average pitch, as in the [2] option.
...
(defmethod! cognitive-sonance ((midicents list) (intervals list))
  ...
  :doc
"Sonance, a vector from cognitive  “maximum consonance” to “maximum
dissonance”, evaluates the contiguous dyads of the input file(s) on the
...
(defmethod! soal-multilists-flat ((information list))
         :doc
"From embedded lists formatted (((onset)(data)...)))), extracts the 'data' and flats it. Intended to adapt the ouput of 'notes-per-onset' and 'velocity-per-onsetí functions, before connecting them to the ësegmentation-midií function. When outputs 1 and 2 merge all the MIDI files data, outputs 3 and 4 preserve the ëmulti-listí standard SOAL format ((...)). "
